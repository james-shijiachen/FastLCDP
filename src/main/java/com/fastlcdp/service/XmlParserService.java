...
public class XmlParserService {
    ...
    private void processTableInheritance(TableDefinition table, Map<String, TableDefinition> tableMap, List<String> processedTables) {
        ...
        if (parentTableName != null && !parentTableName.isEmpty()) {
            ...
            // 合并父表的字段（在子表字段之前）
            List<com.fastlcdp.model.FieldDefinition> mergedFields = new ArrayList<>(parentTable.getFields());
            mergedFields.addAll(table.getFields());
            table.setFields(mergedFields);
            
            // 合并父表的索引
            List<com.fastlcdp.model.IndexDefinition> mergedIndexes = new ArrayList<>(parentTable.getIndexes());
            mergedIndexes.addAll(table.getIndexes());
            table.setIndexes(mergedIndexes);
            
            // 合并父表的关联关系
            List<com.fastlcdp.model.RelationDefinition> mergedRelations = new ArrayList<>(parentTable.getRelations());
