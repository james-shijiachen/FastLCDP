...
import cn.com.traninfo.fastlcdp.erdesigner.model.IndexDefinition;
import cn.com.traninfo.fastlcdp.erdesigner.model.RelationDefinition;
import cn.com.traninfo.fastlcdp.erdesigner.model.TableDefinition;
import cn.com.traninfo.fastlcdp.erdesigner.util.XmlSchemaValidator;
import cn.com.traninfo.fastlcdp.erdesigner.util.XmlValidationResult;
...
public class ComplexDatabaseTest {
    ...
    void testComplexIndexes() {
        ...
        assertEquals("FULLTEXT", fulltextIndex.getType().name());
    ...
    void testComplexRelations() {
        ...
        assertEquals("RESTRICT", selfRelation.getOnDelete().name());
        assertEquals("CASCADE", selfRelation.getOnUpdate().name());
        ...
        assertEquals("CASCADE", userRelation.getOnDelete().name());
    ...
    void testTableTypes() {
        ...
        // 测试抽象表
        TableDefinition baseEntity = tableMap.get("base_entity");
        assertEquals("ABSTRACT", baseEntity.getType().name(), "base_entity应该是抽象表");
        
        TableDefinition businessEntity = tableMap.get("business_entity");
        assertEquals("ABSTRACT", businessEntity.getType().name(), "business_entity应该是抽象表");
        
        TableDefinition organizationBase = tableMap.get("organization_base");
        assertEquals("ABSTRACT", organizationBase.getType().name(), "organization_base应该是抽象表");
        
        // 测试实体表（默认类型）
        TableDefinition departments = tableMap.get("departments");
        assertTrue("ENTITY".equals(departments.getType().name()) || departments.getType() == null,
                "departments应该是实体表（ENTITY或null）");
        
        TableDefinition users = tableMap.get("users");
        assertTrue("ENTITY".equals(users.getType().name()) || users.getType() == null,
                "users应该是实体表（ENTITY或null）");
        
        TableDefinition documents = tableMap.get("documents");
        assertTrue("ENTITY".equals(documents.getType().name()) || documents.getType() == null,
