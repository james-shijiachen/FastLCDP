...
import { CanvasState, Entity, Field, Relationship, RelationshipCategory } from '../types/entity'
...
const emit = defineEmits<{
  ...
  'relationshipRightClick': [relation: Relationship, event: MouseEvent]
  'createFieldRelation': [fromEntityId: string, fromFieldId: string, toEntityId: string, toFieldId: string]
}>()
...
function applyInitialCanvasState(graph: Graph) {
  ...
  // 根据初始的isDragMode状态设置Selection插件
  const selectionPlugin = graph.getPlugin('selection') as Selection
  if (selectionPlugin) {
    if (props.canvasState.isDragMode) {
...
function addSelectionPlugin(graph: Graph) {
  ...
  graph.use(
    new Selection({
      enabled: true,
      multiple: true,
      rubberband: true, // 始终启用框选，通过enable/disable控制
      showNodeSelectionBox: true, // 始终显示选择框，通过enable/disable控制
...
function registerMarker(){
  ...
  try {
    // 注册自定义箭头 - 1的一侧（两条竖杠）
    ;(Graph as any).registerMarker('one-marker', (args: any) => {
      return {
        tagName: 'path',
        d: 'M 8,-8 L 8, 8 M 16, -8 L 16, 8',
        strokeWidth: 2
      }
    })
  } catch (error) {
    // marker 已存在，忽略错误
    console.debug('one-marker already registered')
  }

  try {
    // 注册自定义箭头 - 多的一侧（分叉线）
    ;(Graph as any).registerMarker('many-marker', (args: any) => {
      return {
        tagName: 'path',
        d: 'M 10,0 L 0,-8 M 10,0 L 0,8',
        strokeWidth: 1
      }
    })
  } catch (error) {
    // marker 已存在，忽略错误
    console.debug('many-marker already registered')
  }
}
...
function createRelationshipEdges() {
  ...
  currentEdgeMap.value.forEach(edge => {
    if (!currentRelationshipIds.has(edge.id)) {
      ...
      if (connector && connector.name === 'jumpover' && edge.isVisible()) {
        // 重新设置连接器以触发路径重计算
        edge.setConnector('jumpover', connector.args)
        edge.show()
      }
    }
  })
...
function createRelationshipEdge(relationship: Relationship) {
  ...
  try {
    ...
    switch (relationship.category) {
      case RelationshipCategory.ONE_TO_ONE:
        sourceMarker = 'one-marker'
        targetMarker = 'one-marker'
        break
      case RelationshipCategory.ONE_TO_MANY:
        sourceMarker = 'one-marker'
        targetMarker = 'many-marker'
        break
      case RelationshipCategory.MANY_TO_ONE:
        sourceMarker = 'many-marker'
        targetMarker = 'one-marker'
        break
      case RelationshipCategory.MANY_TO_MANY:
        sourceMarker = 'many-marker'
        targetMarker = 'many-marker'
        break
      default:
    ...
    console.log('createRelationshipEdge', sourceMarker, targetMarker)
    const edge = currentGraph.value.addEdge({
      ...
      attrs: {
        line: {
          ...
          strokeWidth: 1,
          sourceMarker: {name: sourceMarker},
      ...
      router: {
        ...
        args: {
          padding: {
            left: 50,
            right: 50,
            top: 10000, // 设置很大的值阻止从上方连接
            bottom: 10000 // 设置很大的值阻止从下方连接
          }
        }
      }
    })
...
function updateRelationshipEdge(relationship: Relationship, edge: any) {
  ...
  if (sourceNode && targetNode) {
    ...
    console.log('updateRelationshipEdge', sourceMarker, targetMarker)
    // 更新箭头样式
    const lineAttrs: any = {
      stroke: props.isDarkTheme ? '#ffffff' : '#24292e',
      strokeWidth: 1,
...
function setupEventListeners(graphInstance: Graph) {
  ...
  graphInstance.on('cell:contextmenu', ({ cell, e }: { cell: any, e: any }) => {
    if (cell.isNode()) {
      ...
      const relation = props.relationships.find((relation) => relation.id === cell.id)
      if (relation) {
        emit('relationshipRightClick', relation, e as MouseEvent)
      }
    }
  })
  ...
  // 用于跟踪当前高亮的端口
  let currentHighlightedPort: { cellId: string, portId: string } | null = null
  let isConnecting = false

  // 高亮字段端口的函数
  function highlightFieldPort(cell: any, portId: string) {

    if (currentHighlightedPort && 
        (currentHighlightedPort.cellId !== cell.id || currentHighlightedPort.portId !== portId)) {
      // 清除之前的高亮
      const prevCell = graphInstance.getCellById(currentHighlightedPort.cellId)
      if (prevCell && prevCell.isNode() && !isConnecting) {
        prevCell.portProp(currentHighlightedPort.portId, 'attrs/fieldBody/fill', 'transparent')
        prevCell.portProp(currentHighlightedPort.portId, 'attrs/fieldBody/stroke', 'none')
        prevCell.portProp(currentHighlightedPort.portId, 'attrs/fieldBody/strokeWidth', 0)
      }
    }

    // 设置新的高亮
    const port = cell.getPort(portId)
    if (port && port.group === 'fieldPort' && !isConnecting) {
      // 普通悬停效果 - 温和的蓝色高亮
      cell.portProp(portId, 'attrs/fieldBody/stroke', '#FEB663')
      cell.portProp(portId, 'attrs/fieldBody/strokeWidth', 4) // 适中的边框
      cell.portProp(portId, 'attrs/fieldBody/strokeDasharray', '0') // 实线
      currentHighlightedPort = { cellId: cell.id, portId }
    }
  }

  // 清除字段端口高亮的函数
  function clearFieldPortHighlight() {
    if (currentHighlightedPort && !isConnecting) {
      const cell = graphInstance.getCellById(currentHighlightedPort.cellId)
      if (cell && cell.isNode()) {
        // 恢复原始样式
        cell.portProp(currentHighlightedPort.portId, 'attrs/fieldBody/fill', 'transparent')
        cell.portProp(currentHighlightedPort.portId, 'attrs/fieldBody/stroke', 'none')
        cell.portProp(currentHighlightedPort.portId, 'attrs/fieldBody/strokeWidth', 0)
        cell.portProp(currentHighlightedPort.portId, 'attrs/fieldBody/strokeDasharray', 'none')
      }
      currentHighlightedPort = null
    }
  }

  // 使用原生DOM事件监听鼠标移动，实时检测字段端口
  const canvasContainer = graphInstance.container
  let mouseMoveHandler: ((e: MouseEvent) => void) | null = null
  
  const setupMouseMoveTracking = () => {
    if (mouseMoveHandler) {
    ...
    mouseMoveHandler = (e: MouseEvent) => {
      ...
      // 检查是否在节点内
      const cellElement = target.closest('.x6-node')
      if (!cellElement) {
        clearFieldPortHighlight()
        return
      }
      ...
      const cell = graphInstance.getCellById(cellId)
      if (!cell || !cell.isNode()) return
      
      // 方法1：通过DOM查找端口
      let portElement = target.closest('[port]')
      let portId = portElement?.getAttribute('port')
      
      // 方法2：如果方法1失败，通过坐标计算端口
      if (!portId) {
        ...
        // 检查是否在字段区域内
        if (relativeX >= 0 && relativeX <= cellRect.width && relativeY >= 30) { // 30是HEADER_HEIGHT
          const fieldIndex = Math.floor((relativeY - 30) / 25) // 25是LINE_HEIGHT
          const ports = cell.getPorts()
          const fieldPorts = ports.filter((p: any) => p.group === 'fieldPort')
          
          if (fieldIndex >= 0 && fieldIndex < fieldPorts.length) {
            portId = fieldPorts[fieldIndex].id
          }
        }
      }
      
      if (portId) {
        highlightFieldPort(cell, portId)
      } else {
        clearFieldPortHighlight()
      }
    }
  ...
  // 监听连线删除事件
  graphInstance.on('edge:removed', ({ cell }) => {
    if (cell.isEdge()) {
      isConnecting = false
      clearFieldPortHighlight()
    }
  ...
  // 监听端口连接事件（字段拖拽连线）
  graphInstance.on('edge:connected', ({ edge }: { edge: any }) => {
    ...
    // 检查是否是字段端口之间的连接
    if (source && target && source.port && target.port) {
      const sourcePortId = source.port as string
      const targetPortId = target.port as string
      
      // 解析端口ID获取实体ID和字段ID
      const sourceMatch = sourcePortId.match(/^(.+)-(.+)$/)
      const targetMatch = targetPortId.match(/^(.+)-(.+)$/)
      
      if (sourceMatch && targetMatch) {
        const fromEntityId = sourceMatch[1]
        const fromFieldId = sourceMatch[2]
        const toEntityId = targetMatch[1]
        const toFieldId = targetMatch[2]
        
        // 移除临时创建的边，让父组件处理关系创建
        graphInstance.removeCell(edge)
        
        // 发出创建字段关系事件
        emit('createFieldRelation', fromEntityId, fromFieldId, toEntityId, toFieldId)
      }
    }
  })

  // 监听端口拖拽开始事件
  graphInstance.on('edge:mousedown', ({ edge, e }: { edge: any, e: MouseEvent, x: number, y: number, view: any }) => {
    // 为拖拽中的连线添加特殊样式
    if (edge && edge.isEdge()) {
      edge.attr({
        line: {
  ...
  // 监听画布鼠标移动，当鼠标不在任何节点上时清除高亮
  graphInstance.on('blank:mousemove', () => {
    clearFieldPortHighlight()
  })
...
function calculateRelationPathAnchor(sourceNode: Node, targetNode: Node) {
  ...
  const isSourceAboveTarget = sourceCenterY < targetCenterY

  // 计算节点间的垂直距离阈值
  const verticalDistanceThreshold = sourceBBox.height / 2 + targetBBox.height / 2 + 100
  const isNodesCloseVertically = Math.abs(sourceCenterY - targetCenterY) < verticalDistanceThreshold

  if(isSourceLeftOfTarget){
    if(isSourceAboveTarget){
      if(isNodesClose){
        if(isNodesCloseVertically){
          return { 
            source: 'left',
            target: 'right',
            isJumpOver: true
...
onMounted(() => {
  ...
  onBeforeUnmount(() => {
    // 清理鼠标事件监听器
    if (mouseMoveHandler && graph.value?.container) {
      graph.value.container.removeEventListener('mousemove', mouseMoveHandler)
    }
